...
const History = ({ darkMode }) => {
  ...
  useEffect(() => {
    ...
    req.onreadystatechange = () => {
      if (req.readyState == XMLHttpRequest.DONE) {
        if (req.responseText.includes('name')) {
          const res = JSON.parse(req.responseText)
  ...
  useEffect(() => {
    if (fetchComplete === true) {
      // THE INDEX INSIDE THE MAP FUNCTION HERE IS TO LIMIT REQUESTS CAUSE I KEEP GETTING THE ERROR SAYING TOO MANY REQUESTS :(
      binIDS.map((binID, index) => index < 5 && readBin(binID))
